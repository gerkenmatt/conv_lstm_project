...
from . import plot_utils as plotter
...
class MyModel():
    ...
    def build_functional_model(self, configs, kernel_size):
        ...
        feat_extract0 = Conv1D(
            filters=filter_num, 
            kernel_size=kernel_size, 
            input_shape=(sequence_len, 1), 
            padding='same' )(inputs)

        feat_extract1 = Conv1D(
            filters=filter_num, 
            kernel_size=kernel_size, 
            input_shape=(sequence_len, 1), 
            padding='same' )(inputs)
        dropout0 = Dropout(dropout, input_shape=(sequence_len, filter_num))(feat_extract1)

        # lstm network
        lstm1 = LSTM(100, input_shape=(5,filter_num), return_sequences=True)(dropout0)
        dropout1 = Dropout(dropout)(lstm1)
        lstm2 = LSTM(100, return_sequences=False)(dropout1)
        # lstm3 = LSTM(100, return_sequences=False)(dropout1)
        # dropout2 = Dropout(0.2)(lstm3)
        predictions = Dense(1, activation='linear')(lstm2)
    ...
    def train(self, x, y, epochs, batch_size, save_dir, modelType):
        ...
        if modelType == ModelType.FUNCTIONAL: 
            fit_callback = self.func_model.fit(
                x,
                y,
                epochs=epochs,
            ...
            loss_history = fit_callback.history['loss']
            plotter.plot_data(loss_history, "loss history")
            print("loss history: ", str(loss_history) )
        elif modelType == ModelType.SEQUENTIAL: 
            fit_callback = self.seq_model.fit(
                x,
                y,
                epochs=epochs,
                batch_size=batch_size,
